...
def home():
    ...
    if request.method == 'POST' and search_form.search_string.data and search_form.validate_on_submit():
        ...
        if queries.query_is_unique(DATABASE, 'uniprot_id','kinase_info', 'uniprot_id LIKE "{0}" OR name_human LIKE "{0}" OR prot_name LIKE "{0}"'.format(requested_name)) : # modify: get list of kinases
            uniprot_id = queries.select_where(DATABASE, 'uniprot_id','kinase_info', 'uniprot_id LIKE "{0}" OR name_human LIKE "{0}" OR prot_name LIKE "{0}"'.format(requested_name)).loc[0,'uniprot_id']
            ...
            uniprot_id = queries.select_where(DATABASE, 'uniprot_id','kinase_info', 'uniprot_id LIKE "%{0}%"'.format(requested_name)).loc[0,'uniprot_id']
            url = '/kinase/' + uniprot_id
            return redirect(url)
...
def kinase_search_result():
    ...
    search_results = queries.select_where(DATABASE, 'uniprot_id, name_human, chromosome, fasd_name, ensembl_gene_id','kinase_info',\
...
def kinase_data(kin_name):
    ...
    if queries.query_is_unique(DATABASE, 'uniprot_id', 'kinase_info', "uniprot_id LIKE '{}'".format(kin_name)) : # modify: get list of kinases
        kin_name = kin_name
        gral_info = queries.select_where(DATABASE, '*', 'kinase_info', 'uniprot_id LIKE "{}"'.format(kin_name))
        isoforms = list(queries.select_where(DATABASE, 'isoform', 'isoforms', 'uniprot LIKE "{}"'.format(kin_name)).loc[:,'isoform'])
        ...
        reactions_list = list(queries.select_where(DATABASE, 'reaction_text', 'reactions', 'uniprot LIKE "{}"'.format(kin_name)).loc[:,'reaction_text'])
        cell_loc_list= list(queries.select_where(DATABASE, 'subcell_location', 'subcell_location', 'uniprot LIKE "{}"'.format(kin_name)).loc[:,'subcell_location'])

        targets = queries.select_where(DATABASE, 'sub_acc_id, sub_gene, sub_mod_rsd, site_7_aa', 'kinase_substrate', 'kin_acc_id LIKE "{}"'.format(kin_name))

        phosphosites = queries.select_where(DATABASE, 'residue_position, modif, type_modif, genom_begin, genom_end', 'phosphosites', 'uniprot_id LIKE "{}"'.format(kin_name))

        cell_loc_add_text_list_tmp = list(queries.select_where(DATABASE, 'subcell_aditional_text', 'subcell_location_text', 'uniprot LIKE "{}"'.format(kin_name)).loc[:,'subcell_aditional_text'])
        ...
        diseases = queries.select_where(DATABASE, 'DISTINCT disease_name, effect_text, disease_description', 'diseases', 'uniprot LIKE "{}" AND disease_name NOT LIKE "" ORDER BY disease_name'.format(kin_name))
...
def fasta_protein(kin_name):
    if queries.query_is_unique(DATABASE, 'uniprot_id', 'kinase_info','uniprot_id  LIKE "{}"'.format(kin_name)):
        text = queries.select_where(DATABASE, 'prot_sequence, chromosome, reverse','kinase_info', 'uniprot_id  LIKE "{}"'.format(kin_name))
        ...
        text = queries.select_where(DATABASE, 'prot_sequence, chromosome, reverse','isoforms_info', 'uniprot_id  LIKE "{}"'.format( kin_name))
        sequence = text.loc[0,'prot_sequence']
        divide_each = 40  # modify: change size !!
        seq_size = len(sequence)
        list_range = range(0,seq_size,divide_each)
...
def fasta_gene(kin_name):
    if queries.query_is_unique(DATABASE, 'uniprot_id', 'kinase_info','uniprot_id  LIKE "{}"'.format( kin_name)) : # modify: get list of kinases
        text = queries.select_where(DATABASE, 'chromosome, genome_sequence, reverse, ensembl_gene_id, genome_starts, genome_ends','kinase_info', 'uniprot_id  LIKE "{}"'.format( kin_name))
...
def api_all(kin_name):
    if queries.query_is_unique(DATABASE, 'uniprot_id', 'kinase_info','uniprot_id', kin_name) : # modify: get list of kinases
        q_output = queries.select_where(DATABASE, '*','kinase_info', 'uniprot_id  LIKE "{}"'.format(kin_name))
